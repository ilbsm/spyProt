...
class PfamAnnotation(AnnotationBase):
    ...
    def __init__(self, pdb, chain='A'):
        ...
        url = urllib.urlopen(Uni + pdb)
        data = json.loads(url.read().decode('utf-8'))
        self.uniprot = list(data[pdb]['UniProt'].keys())[0]

        url = urllib.urlopen(PDBe + self.uniprot)
        data = json.loads(url.read().decode('utf-8'))
        if data[self.uniprot]['Pfam'].keys() != []:
            ...
            for e in set(self.domains.values()):
                ...
                for i in k:
                    self.results.append((pdb, chain, str(self.pfam_domains[e]['name'].decode('utf-8')),
                                         str(self.pfam_domains[e]['description'].decode('utf-8')), str(e.decode('utf-8')), i, self.coverage[i][e]))
        self.uniprot = str(self.uniprot.decode('utf-8'))
