...
def get_region(seq1, seq2):
    ...
    try:
        alignments = pairwise2.align.localds(seq1, seq2, matrix, -10, -0.5) #czemu akurat te dane --> czy zmiana mialaby wplyw na alignment i przypisaną domene?
        #print("al",alignments)
        if alignments:
            ...
            if region[-1] == "-":
                ...
                j=j*(-1)
                region = region[:j]
            #print(alignments[0], i+1, len(protein)-j*(-1))
            #print("alignments",alignments)
            #print("region",region, i+1, len(protein)-j*(-1))
...
def nc_cuts_from_knotprot(pdb_id, chain_id): # find N and C cut of the entanglement region
    candidate_files = ["_nr_chains_knotted_N_C", "_red_chains_knotted_N_C", "_all_chains_knotted_N_C"] #??czym sa elementy
    entanglement = {}
    for file in candidate_files:
        with Session() as s:
            site=s.get("https://knotprot.cent.uw.edu.pl/{}".format(file), allow_redirects=False) #po wydrukowaniu site --> lista struktur i info
            #print ("site_text",file,site.text)
...
def seq_structure_from_knotprot(pdb_id, chain_id): # returns the sequence of the structure that was used to compute entanglement in KnotProt; it includes only resolved amino acids! no gaps!
    with Session() as s:
        site=s.get("https://knotprot.cent.uw.edu.pl/sequence/{}/{}/".format(pdb_id, chain_id), allow_redirects=False)
        #print('seq_structure', "https://knotprot.cent.uw.edu.pl/sequence/{}/{}/".format(pdb_id, chain_id) )
        if site.text:
...
def seq_from_uniprot(pdb_id): # returns fasta of the whole protein, not only the sequence from the structure, sometimes there is more than one sequence
    with Session() as s:
        site=s.get("https://www.uniprot.org/uniprot/?query=database:(type:pdb {})&format=fasta".format(pdb_id), allow_redirects=False)
        #print('site_uniprot',"https://www.uniprot.org/uniprot/?query=database:(type:pdb {})&format=fasta".format(pdb_id))
        if site.text:
            #print("seq",site.text.split(">"))
...
#get_entangled_region mozna zamienic na wyszukanie bazy 'knots_info' 
def get_entangled_region(pdb_id, chain_id):
    protein_id = seq_from_uniprot(pdb_id)
    seq_protein = fasta2string(protein_id) #fasta file
    seq_structure = seq_structure_from_knotprot(pdb_id, chain_id) # returns the sequence of the structure that was used to compute entanglement in KnotProt
    knotprot_ends = nc_cuts_from_knotprot(pdb_id, chain_id)
    entangled_regions = {}
    for knot in knotprot_ends:
        knotprot_N_end = knotprot_ends[knot][0]
        knotprot_C_end = knotprot_ends[knot][1]
        seq_structure_with_gaps, start_region, stop_region = get_region(seq_protein, seq_structure)
    ...
    #print('entangled', entangled_regions)
    return(entangled_regions)

def get_entangled_domains(dict_knot_ranges, dict_domain_ranges): #dict_domain_ranges={'PF01746': [22, 221]}; dict_knot_ranges={'K31': [89, 132]}
    coverage = {}
    domains = {}
...
class PfamAnnotation(AnnotationBase):
    ...
    def __init__(self, pdb, chain='A'):
        ...
        #print(self.first_file) #/home/mnguyen/.local/spyprot/pdb_chain_pfam.tsv.gz
        ...
        with gzip.open(self.first_file) as s:
            for line in s.readlines():
                ...
                z = q.match(line)
                if z:
                    self.uniprot=str(z.group(0)).split('\t')[2]
                    #print(self.uniprot)
                    break
        """
        ...
        with urllib.request.urlopen(PDBe+self.uniprot) as url:
            ...
            if data[self.uniprot]['Pfam'].keys()!=[]:
                for i in data[self.uniprot]['Pfam'].keys():
                    self.pfam_domains[i]=data[self.uniprot]['Pfam'][i]  #pfam_domain --> {pfam: {mappinngs, identifier, description}}
                    #print(self.pfam_domains[i]['mappings'][0]['unp_start'])
                    self.pfam_mappings[i]=[self.pfam_domains[i]['mappings'][0]['unp_start'],self.pfam_domains[i]['mappings'][0]['unp_end']]

                self.entangled_regions = get_entangled_region(pdb, chain)
                self.coverage, self.domains = get_entangled_domains(self.entangled_regions, self.pfam_mappings)
                ...
                for e in set(self.domains.values()):
                        ...
                        for i in k:
                            #print(self.coverage[i][e])
                            self.results.append((pdb, chain, self.pfam_domains[e]['name'], self.pfam_domains[e]['description'], e, i, self.coverage[i][e]))
                        #self.results.append((pdb, chain, a,b, e, self.coverage[e]))
                print(self.results)
            
    def which_knot(self,ent_domains, search):
        kn=[]
